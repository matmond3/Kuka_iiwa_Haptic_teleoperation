...
import math
import numpy as np
from RPYtoRotation import RPYtoRotation
...
try:
    ...
    jPos=np.deg2rad([8,28.23,4.36,-118.46,-3,34.65,0])
    #jPos = np.deg2rad([0,0,0,0,0,0,0])
    ...
    EEF_Pose = iiwa.getEEFPos()
    print("Cartesian Position")
    print(EEF_Pose)
    kuka_Position = np.array([EEF_Pose[0] / 1000, EEF_Pose[1] / 1000, EEF_Pose[2] / 1000])
    kuka_RPY = np.array([EEF_Pose[3], EEF_Pose[4], EEF_Pose[5]])
    kuka_R = RPYtoRotation(kuka_RPY)

    T = np.identity(4)
    T[0:3,0:3] = kuka_R.T
    T[0:3,3] = -np.matmul(kuka_R.T,kuka_Position.T)
